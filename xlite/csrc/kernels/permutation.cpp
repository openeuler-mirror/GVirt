...
__aicore__ inline void permutation1(GM_ADDR input, GM_ADDR routing_map, GM_ADDR output,
                                    GM_ADDR unp_idx, GM_ADDR counts,
                                    ...
                                    uint32_t experts_start_idx, uint32_t experts_end_idx)
{
    ...
    __ubuf__ uint32_t *routing_count = (__ubuf__ uint32_t*)get_imm(n_routed_experts * sizeof(uint32_t) + ROUND_UP(TOKEN_TILE * n_routed_experts / 8, BLOCK_SIZE));
    ...
    vector_dup(c, 0, DIV_ROUND_UP(n_routed_experts * sizeof(uint32_t), VECTOR_MAX_BYTESIZE), 1, 1, 8, 1);
    pipe_barrier(PIPE_V);
    ...
    for (uint32_t token_start = 0; token_start < n_tokens; token_start += TOKEN_TILE) {
        ...
        uint32_t token_tile = (token_start + TOKEN_TILE > n_tokens) ? n_tokens - token_start : TOKEN_TILE;
        copy_gm_to_ubuf(routing, routing_map_tile, 0, 1, DIV_ROUND_UP(token_tile * n_routed_experts / 8, BLOCK_SIZE), 0, 0);
        ...
        for (uint32_t curr_expert = local_start; curr_expert < local_end; curr_expert++) {
            ...
            GM_ADDR unp_idx_tile = unp_idx + curr_expert * n_tokens * sizeof(uint32_t) + token_start * sizeof(uint32_t);
            set_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
            wait_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
            copy_ubuf_to_gm_align_b32(unp_idx_tile, routing_count, 0, 1, token_tile * sizeof(uint32_t), 0, 0, 0, 0);
            set_flag(PIPE_MTE3, PIPE_S, EVENT_ID0);
        }
        wait_flag(PIPE_MTE3, PIPE_S, EVENT_ID0);
    }
    ...
    if (local_end > local_start) {
        ...
        copy_ubuf_to_gm_align_b32(counts_local, c, 0, 1, (local_end - local_start) * sizeof(uint32_t), 0, 0, 0, 0);
    }
                                    ...
                                    uint32_t experts_start_idx, uint32_t experts_end_idx)
{
    uint32_t sum = 0;
    __ubuf__ uint32_t *c = (__ubuf__ uint32_t *)get_imm(0);
    __ubuf__ uint32_t *s = (__ubuf__ uint32_t *)get_imm(ROUND_UP(n_routed_experts * sizeof(uint32_t), VECTOR_MAX_BYTESIZE));
    __ubuf__ uint32_t *psum = (__ubuf__ uint32_t *)get_imm(2 * ROUND_UP(n_routed_experts * sizeof(uint32_t), VECTOR_MAX_BYTESIZE));

    copy_gm_to_ubuf(c, counts, 0, 1, DIV_ROUND_UP(n_routed_experts * sizeof(uint32_t), BLOCK_SIZE), 0, 0);
    set_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
    wait_flag(PIPE_MTE2, PIPE_S, EVENT_ID0);
    ...
    *psum = sum;
    ...
    set_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
    wait_flag(PIPE_S, PIPE_MTE3, EVENT_ID0);
    copy_ubuf_to_gm_align_b32(unp_idx + n_routed_experts * n_tokens * sizeof(uint32_t), s, 0, 1, n_routed_experts * sizeof(uint32_t), 0, 0, 0, 0);
                                    ...
                                    uint32_t experts_start_idx, uint32_t experts_end_idx)
{
    ...
    __ubuf__ uint64_t *routing2 = (__ubuf__ uint64_t *)get_imm(routing1 + ROUND_UP(n_routed_experts / 8, BLOCK_SIZE) / sizeof(uint64_t));
    __ubuf__ uint64_t *routing[2] = {routing1, routing2};
    __ubuf__ uint8_t *row1 = (__ubuf__ uint8_t *)get_imm(routing2 + ROUND_UP(n_routed_experts / 8, BLOCK_SIZE) / sizeof(uint64_t));
    __ubuf__ uint8_t *row2 = (__ubuf__ uint8_t *)get_imm(row1 + ROUND_UP(dim * sizeof(bfloat16_t), BLOCK_SIZE));
    ...
    for (curr_token = block_idx; curr_token < n_tokens; curr_token += block_num) {
        ...
        copy_gm_to_ubuf(routing[curr], (__gm__ uint8_t *)routing_map + (curr_token * n_routed_experts / 8), routing_dmi_config);
        ...
        for (curr_expert = experts_start_idx; curr_expert < experts_end_idx; curr_expert++) {
            ...
            if (first_iter) {
                copy_gm_to_ubuf(row[curr], input + (curr_token) * dim * sizeof(bfloat16_t), dmi_config);
                set_flag(PIPE_MTE2, PIPE_MTE3, curr);
                wait_flag(PIPE_MTE2, PIPE_MTE3, curr);
                first_iter = 0;
            }
            uint32_t off_T = curr_expert * n_tokens * sizeof(uint32_t) + curr_token * sizeof(uint32_t);
            uint32_t j = off_T == 0 ? 0 : *((__gm__ uint32_t *)(unp_idx + off_T));
            copy_ubuf_to_gm(output + (s[curr_expert] + j) * dim * sizeof(bfloat16_t), row[curr], dmi_config);
        }
...
extern "C" __global__ __aicore__ void permutation(GM_ADDR input, GM_ADDR routing_map, GM_ADDR output,
                                                  GM_ADDR unp_idx, GM_ADDR counts,
                                                  uint32_t n_tokens, uint32_t dim, uint32_t n_routed_experts,
                                                  uint32_t experts_start_idx, uint32_t experts_end_idx)
{
    ...
    permutation1(input, routing_map, output, unp_idx, counts, n_tokens, dim,
                 n_routed_experts, experts_start_idx, experts_end_idx);
    ffts_cross_core_sync(PIPE_MTE3, config);
    wait_flag_dev(flag_id);

    if (block_idx == 0) {
        permutation2(input, routing_map, output, unp_idx, counts, n_tokens, dim,
                     n_routed_experts, experts_start_idx, experts_end_idx);
    }
    ffts_cross_core_sync(PIPE_MTE3, config);
    wait_flag_dev(flag_id);

    permutation3(input, routing_map, output, unp_idx, counts, n_tokens, dim,
                 n_routed_experts, experts_start_idx, experts_end_idx);
}
#endif