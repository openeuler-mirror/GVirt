...
extern "C" __global__ __aicore__ void silu_and_mul_bfloat16_t(GM_ADDR x, GM_ADDR y, GM_ADDR pm, int n_tokens, int dim)
{
    ...
    int step = dim * 2;
    int dim_split = dim;
    int max_dim = MAX_REPEAT_TIMES * calc_pad; //支持ping-pong
    int split = 1;
    ...
    __ubuf__ float* output_ub = reinterpret_cast<__ubuf__ float*>(tmp + padded_dim);
    ...
    for (int index = block_idx; index < n_tokens * split; index += block_num) {
        ...
        vmul(output_ub, tmp, x32_ub + padded_dim, repeat_cal, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
