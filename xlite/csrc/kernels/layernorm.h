...
__aicore__ inline void layernorm(GM_ADDR inout, GM_ADDR weight, GM_ADDR bias, GM_ADDR out,
                                 ...
                                 uint32_t in_start_offset, uint32_t out_start_offset)
{
    ...
    off += inout_blocksize;
    auto bias_addr = reinterpret_cast<__ubuf__ Dtype *>((uintptr_t)off);
    off += inout_blocksize;
    ...
    for (uint32_t loop = block_idx, ping = 1; loop < loop_count; loop += block_num) {
        ...
        // (x - mean) / sqrt(sum((x - mean) ^ 2) + eps)
        vdiv(calc1, calc0, calc1, repeat, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);

        // (x - mean) / sqrt(sum((x - mean) ^ 2) + eps) * weight
...
#define LAYERNORM_FUNC_DEFINE(dtype)                                                             \
    extern "C" __global__ __aicore__ void layernorm_##dtype(                                     \
        GM_ADDR inout, GM_ADDR weight, GM_ADDR bias, GM_ADDR out, uint32_t token_num,            \
    ...
    {                                                                                            \
        layernorm<dtype>(inout, weight, bias, out, token_num, norm_dim, norm_eps, cnt_per_token, \
