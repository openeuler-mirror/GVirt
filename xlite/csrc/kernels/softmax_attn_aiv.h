    ...
    for (int idx = 0; idx < m; ++idx) {
        ...
        for (int block = 0; block < subBlockNum; block++) {
            ...
            if (curLen * sizeof(Dtype) % BLOCK_SIZE == 0) {
                copy_gm_to_ubuf(in, buf + offset, 0, 1, DIV_ROUND_UP(curLen * sizeof(Dtype), BLOCK_SIZE), 0, 0);
            } else {
                copy_gm_to_ubuf_align_b16(in, buf + offset, 0, 1, curLen * sizeof(Dtype), 0, 0, 0, 0);
            }
            ...
            if (curLen * sizeof(Dtype) % BLOCK_SIZE == 0) {
                copy_ubuf_to_gm(buf + offset, out, 0, 1, DIV_ROUND_UP(curLen * sizeof(Dtype), BLOCK_SIZE), 0, 0);
            } else {
                copy_ubuf_to_gm_align_b16(buf + offset, out, 0, 1, curLen * sizeof(Dtype), 0, 0, 0, 0);
            }
            set_flag(PIPE_MTE3, PIPE_V, EVENT_ID1);
        }
        ...
        for (int block = 0; block < subBlockNum; block++) {
            ...
            if (block == subBlockNum - 1 && n > actualCalcLen) {
                ...
                if (n - offset > last) {
                    vector_dup(out + last, Dtype(0), (n - offset - last) / pad, 1, 1, 8, 0);
                    pipe_barrier(PIPE_V);
                }
                curLen = n - offset;
            }
