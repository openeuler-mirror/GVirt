    ...
    uint32_t block_size, GM_ADDR key, GM_ADDR kcache, GM_ADDR vcache, GM_ADDR slot_mapping)
{
    ...
    __ubuf__ float *t = (__ubuf__ float *)get_imm(0);
    __ubuf__ Dtype *input = (__ubuf__ Dtype *)get_imm(1 << 10);
    __ubuf__ Dtype *ouput = (__ubuf__ Dtype *)get_imm(2 << 10);
    __ubuf__ float *freqs = (__ubuf__ float *)get_imm(3 << 10);
    __ubuf__ float *t0 = (__ubuf__ float *)get_imm(4 << 10);
    __ubuf__ float *t1 = (__ubuf__ float *)get_imm(5 << 10);
    __ubuf__ float *t2 = (__ubuf__ float *)get_imm(6 << 10);
    __ubuf__ float *t3 = (__ubuf__ float *)get_imm(7 << 10);
    __ubuf__ float *t4 = (__ubuf__ float *)get_imm(8 << 10);
    __ubuf__ float *t5 = (__ubuf__ float *)get_imm(9 << 10);
    __ubuf__ float *t6 = (__ubuf__ float *)get_imm(10 << 10);
    __ubuf__ float *t7 = (__ubuf__ float *)get_imm(11 << 10);
    __ubuf__ float *t8 = (__ubuf__ float *)get_imm(12 << 10);
    __ubuf__ Dtype *kinput = (__ubuf__ Dtype *)get_imm(13 << 10);
    ...
    for (int index = block_idx; index < nTokens * nLocalHeads; index += block_num) {
        ...
        if (need_k_cache) {
            ...
            copy_gm_to_ubuf(kinput, (__gm__ Dtype *)(key) + index * kdim, 0, 1,
        ...
        pipe_barrier(PIPE_V);
        vsub(t, t5, t6, 1 /*repeats*/, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);
        ...
        pipe_barrier(PIPE_V);
        vadd(t + qkRopeHeadDim / 2, t7, t8, 1 /*repeats*/, 1, 1, 1, 8, 8, 8);
        pipe_barrier(PIPE_V);

        wait_flag(PIPE_MTE3, PIPE_V, EVENT_ID0);
        if constexpr (std::is_same_v<Dtype, float16_t>) {
            vconv_f322f16(ouput, t, 2, 1, 1, 4, 8);
        } else if constexpr (std::is_same_v<Dtype, bfloat16_t>) {
            vconv_f322bf16r(ouput, t, 2, 1, 1, 4, 8);
        }
