...
__aicore__ void unpermutation(GM_ADDR input, GM_ADDR routing_map, GM_ADDR output,
                              ...
                              uint32_t experts_start_idx, uint32_t experts_end_idx)
{
    ...
    uint64_t weights_dmi_config = __set_dmi_config(0, 1, DIV_ROUND_UP(n_routed_experts * sizeof(bfloat16_t), BLOCK_SIZE), 0, 0);
    ...
    if constexpr (std::is_same<Dtype, bfloat16_t>::value) {
        w_float_addr = (__ubuf__ uint8_t *)get_imm(ROUND_UP(dim * sizeof(bfloat16_t), VECTOR_MAX_BYTESIZE) +
        ...
        w_bf16_addr = (__ubuf__ uint8_t *)get_imm(ROUND_UP(dim * sizeof(bfloat16_t), VECTOR_MAX_BYTESIZE) +
                                                            3 * ROUND_UP(dim * sizeof(float), VECTOR_MAX_BYTESIZE) +
                                                            ROUND_UP(dim * sizeof(bfloat16_t), VECTOR_MAX_BYTESIZE) +
                                                            ROUND_UP(n_routed_experts, VECTOR_MAX_BYTESIZE) +
    ...
    for (curr_token = block_idx; curr_token < n_tokens; curr_token += block_num) {
        ...
        if constexpr (std::is_same<Dtype, bfloat16_t>::value) {
            copy_gm_to_ubuf((__ubuf__ bfloat16_t *)w_bf16_addr,
                            (__gm__ bfloat16_t *)(weights_map + curr_token * n_routed_experts * sizeof(bfloat16_t)),
                            weights_dmi_config);
        }
        ...
        if constexpr (std::is_same<Dtype, bfloat16_t>::value) {
            ...
            vconv_bf162f32((__ubuf__ float *)w_float_addr, (__ubuf__ bfloat16_t *)w_bf16_addr, vector_bf162fp32_config);
        ...
        for (uint32_t i = experts_start_idx; i < experts_end_idx; i++) {
            if (bitmapTest(routing, i)) {
                ...
                if constexpr (std::is_same<Dtype, bfloat16_t>::value) {
                    ...
                    w = *((__gm__ float *)(weights_map + (curr_token * n_routed_experts + i) * sizeof(float)));
                }
