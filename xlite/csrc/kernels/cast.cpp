...
__aicore__ void cast_kernel(GM_ADDR x, GM_ADDR y, uint32_t length)
{
    ...
    uint32_t tile_size = length <= max_num * get_block_num() ? DIV_ROUND_UP(length, get_block_num()) : max_num;
    ...
    for (uint32_t process = block_idx; process < process_num; process += uint32_t(block_num)) {
        uint32_t actual_len = length - process * tile_size >= tile_size ? tile_size : length - process * tile_size;
        // bf16 > fp32
        if ((std::is_same<SrcType, bfloat16_t>::value) && (std::is_same<TarType, float>::value)) {
            wait_flag(PIPE_V, PIPE_MTE2, EVENT_ID0);
            copy_gm_to_ubuf(UBA(__bf16)t1, GMA(__bf16)x + process * tile_size, 0, 1, DIV_ROUND_UP(actual_len * sizeof(__bf16), BLOCK_SIZE), 0, 0);
            set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
            wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);

            wait_flag(PIPE_MTE3, PIPE_V, EVENT_ID0);
            vconv_bf162f32(UBA(float)t2, UBA(__bf16)t1, DIV_ROUND_UP(actual_len * sizeof(float), VECTOR_MAX_BYTESIZE), 1, 1, 8, 4);
            set_flag(PIPE_V, PIPE_MTE2, EVENT_ID0);

            set_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
            wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);

            copy_ubuf_to_gm(GMA(float)y + process * tile_size, UBA(float)t2, 0, 1, DIV_ROUND_UP(actual_len * sizeof(float), BLOCK_SIZE), 0, 0);
            set_flag(PIPE_MTE3, PIPE_V, EVENT_ID0);
        }
        ...
        if ((std::is_same<SrcType, float>::value) && (std::is_same<TarType, bfloat16_t>::value)) {
            wait_flag(PIPE_V, PIPE_MTE2, EVENT_ID0);
            copy_gm_to_ubuf(UBA(float)t1, GMA(float)x + process * tile_size, 0, 1, DIV_ROUND_UP(actual_len * sizeof(float), BLOCK_SIZE), 0, 0);
            set_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);
            wait_flag(PIPE_MTE2, PIPE_V, EVENT_ID0);

            wait_flag(PIPE_MTE3, PIPE_V, EVENT_ID0);
            vconv_f322bf16r(UBA(__bf16)t2, UBA(float)t1, DIV_ROUND_UP(actual_len * sizeof(float), VECTOR_MAX_BYTESIZE), 1, 1, 4, 8);
            set_flag(PIPE_V, PIPE_MTE2, EVENT_ID0);

            set_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);
            wait_flag(PIPE_V, PIPE_MTE3, EVENT_ID0);

            copy_ubuf_to_gm(GMA(__bf16)y + process * tile_size, UBA(__bf16)t2, 0, 1, DIV_ROUND_UP(actual_len * sizeof(__bf16), BLOCK_SIZE), 0, 0);
            set_flag(PIPE_MTE3, PIPE_V, EVENT_ID0);
        }
    }
    wait_flag(PIPE_MTE3, PIPE_V, EVENT_ID0);
