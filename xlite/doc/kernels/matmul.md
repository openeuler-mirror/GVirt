...
kernel 入口（`csrc/kernels/matmul.h:477`）：
...
| m, n, k | 标量 | - | uint64_t | GEMM 维度。host 侧由 weight shape 推导：`n = transpose ? weight.shape[1] : weight.shape[0]`，`k = transpose ? weight.shape[0] : weight.shape[1]`（`csrc/op.cpp:650-652`） |
...
int4 路径中 x/y 为 int32 容器（`npu_convert_weight_to_int4pack` 打包结果），`_C.cpp` 中 `View(INT4)` 重解释为 int4（`csrc/_C.cpp:1270-1271`、`csrc/_C.cpp:2022-2023`）。
...
| bfloat16_t | float（host 侧 bf16 bias 先 cast 为 fp32，`csrc/op.cpp:667-671`） | bfloat16_t | `csrc/kernels/matmul_bfloat16_t.cpp` | `matmul_bfloat16_t` |
| float | float | float | `csrc/kernels/matmul_float.cpp` | `matmul_float`（仅 `!transpose`，`csrc/op.cpp:673`） |
| int8_t | int32_t | half | `csrc/kernels/matmul_int8_t.cpp` | `matmul_int8_t`（W8A8：`in/weight INT8 + out FP16`，`csrc/op.cpp:684-685`） |
| int4b_t | int32_t | half | `csrc/kernels/matmul_int4b_t.cpp` | `matmul_int4b_t`（W4A4/W4A8：`in/weight INT4 + out FP16`，`csrc/op.cpp:686-687`） |

另外 host 侧还支持混合精度组合（`csrc/op.cpp:675-683`）：`in BF16 + weight/out FP32`、`in BF16 + weight FP32 + out BF16`（均 `!transpose`），通过前置/后置 cast kernel 走 `matmul_float`。

kernel 任务类型为 `KERNEL_TYPE_AIC_ONLY`（`csrc/kernels/matmul.h:482`），运行在 AI Cube 核上。

## 实现原理

### 整体数据流

单核处理一个或多个 `(m0 x n0)` 输出 tile，单个 tile 内沿 K 维循环。数据流为经典的五级 Cube 流水（`csrc/kernels/matmul.h:204-394` RunTileBody）：
...
- tile 划分：`mLoop = ceil(m/m0)`，`nLoop = ceil(n/n0)`（`csrc/kernels/matmul.h:161-162`），`TaskTilesInit` 返回 `mLoop * nLoop` 个 tile；多核按 `for (idx = GetBlockIdx(); idx < tiles; idx += GetBlockNum())` 交错认领（`matmul.h:197-199`）。
- 默认分块（`m0 == -1` 时）：`m0=128`，`n0 = (hasBias||hasDeqScale) ? 128 : 256`（L1 512KB 限制，bias/deqScale 需要额外 L1/fixpipe 空间时收缩 n0，`matmul.h:40-46`），`k0 = 512*8/dtypeBits`。host 侧默认逻辑在 `csrc/auto_tuner.cpp:212-256`：`m0 = min(ROUND_UP(m,32), 128)`、`n0 = needExtraSpace ? 128 : 256`、`k0 = 4096/dtypeBits`，并在 `totalLoops < 3*aicNum` 且尾核负载不均时进一步缩小 n0（64/128/256/384）并可能减半 k0、缩 m0 到 64，以保证各 AICore 均载。
- K 维两级量化：L1 中 A 的 tile 覆盖 `kDtileSize = 2*k0`（8 个 L0 量化片），L0A/L0B 的单次 Mmad 片为 `kQtileSize = k0/4`（`matmul.h:72-73`），即 A 的 GM→L1 每 8 次 K 循环做一次（`kIdx8`），B 每 4 次一次（`kIdx4`），L0A/L0B 乒乓每 2 次一次（`kIdx2`）。
- 分形块尺寸：`mBlockSize=16`；`nBlockSize=16`（transpose 且量化类型时为 `32*8/dtypeBits`，即 int8 为 32、int4 为 64，对应 B 转置分形单元 32x32x1B）；`kBlockSize = 32*8/dtypeBits`（fp16 为 32、int8 为 32、int4 为 64，`matmul.h:62-70`）。边界 tile 用 `mActual/nActual` 与对应 `ROUND_UP(..., xBlockSize)` 的 pad 值处理（`matmul.h:215-227`）。
- swizzle 重排：`GetMNBlockIdx`（`matmul.h:396-435`）支持两种方向——`swizzleDirection=0`（Zn：沿 M 方向按 `swizzlCount` 分组、组间 N 序蛇形遍历）与 `=1`（Nz：沿 N 方向分组、组间 M 序蛇形遍历）——调整 tile 遍历顺序以提升 cache 命中。host 侧 `XlitePickSwizzle` 按 (n, k) 查表（实测 910B3 上 `{6144,2048}` 与 `{2048,6144}` 均取 `0xe01`，即 count=14、direction=1，`csrc/swizzle.cpp:11-18`）。

### L1/L0 缓冲布局

`Init`（`matmul.h:31-129`）按 tile 尺寸静态规划各 buffer 偏移：
...
`SetFlags`/`WaitFlags`（`matmul.h:131-141`、`166-176`）在 tile 生命周期的起止成对补齐全部事件；tile 内部使用：
...
- `FIX_MTE2`/`MTE2_FIX` EVENT_ID5：deqScale 的 L1↔fixpipe 链路（属于 fixpipe barrier，`matmul.h:265` 注释）；
- `M_FIX`/`FIX_M` EVENT_ID0：L0C 就绪与 fixpipe 搬出完成。

乒乓选择器是 tile 内局部变量（每 tile 从 0 开始），因为 tile 退出时两级缓冲都已释放（`matmul.h:210-211` 注释）。L1A 的乒乓在 `kIdx8==7` 或末轮翻转（`matmul.h:338-340`），L1B 在 `kIdx4==3` 或末轮翻转（`matmul.h:354-356`）。
...
- `weightNZ=True` 时 y 已是 Fractal NZ 格式（`torch_npu.npu_format_cast(..., ACL_FORMAT_FRACTAL_NZ)` 或 `matrix_nd2nz`），kernel 走 `CopyGmToL1` 免去 ND→NZ 转换（`matmul.h:308-311`、`315-318`），节省 MTE2 带宽；float 类型不支持 NZ（测试中 `weight_nz and dtype != torch.float`）。
- bias 仅在量化路径为 int32；浮点路径 MatDtype=float，bf16 调用时 host 先用 `cast_bfloat16_t_float` 把 bias 升精度（`csrc/op.cpp:667-671`）。bias 在首个 K 块通过 `Mmad(c, a, b, bias, params)` 一次进入 L0C（`matmul.h:372-377`）。
