...
| ws | 输入 | [num]（指针数组，int64） | uint64 | 每个元素是专家 i 的权重 GM 地址。host 侧把 `std::vector<at::Tensor>` 的指针 MemcpyH2D 到 device（`csrc/_C.cpp:1844-1847`） |
...
| m0, n0, k0 | 标量 | - | uint64 | 分块尺寸；host 固定传 -1（用 kernel 默认，`csrc/op.cpp:833-836`） |
...
| swizzle | 标量 | - | uint64 | tile 遍历 swizzle（host 传 `rt.defaultMatmulSwizzle`，`csrc/op.cpp:836`） |
...
| float | float | float | `csrc/kernels/group_matmul_float.cpp` | `group_matmul_float`（仅 `!transpose`，`csrc/op.cpp:819-820`） |
...
dtype 分派在 `csrc/op.cpp:814-827`：`in.dtype == INT8 && weightDtype == INT8 && output FP16` 走 int8；`in.dtype == INT4 && weightDtype == INT4 && output FP16` 走 int4。W4A8 场景 Python 侧把 int32 容器的激活与权重 View 成 INT4（`csrc/_C.cpp:1839`、`1857-1859`）。
...
2. `SetFlags()` 一次性建立全部事件旗标（`matmul.h:131-141`）；
3. 遍历专家 `i ∈ [startIdx, endIdx)`：
   ...
   - `TaskTilesInit(x + xOffBytes, w, z + zOffBytes, nullptr, deqScale, kM, kN, kK)` 绑定本专家的 GM 地址与形状，返回 tile 数（`matmul.h:147-163`）；
...
- m0/n0/k0 由 kernel 默认值决定（host 传 -1，`csrc/op.cpp:834`），即 `m0=128`、`n0 = useDequant ? 128 : 256`、`k0 = 512*8/dtypeBits`（`matmul.h:40-46`）。
