...
host 侧 launch 见 `csrc/op.cpp:1274`(`XliteOpSoftmaxTopK`)。kernel 签名(`csrc/kernels/softmax_topk.h:269`):
...
| routingMap | 输出 | `[numTokens, ceil(numRoutedExperts/32)]`(BIT1 打包) | BIT1(uint32 位图) | 每个被选专家置 1 bit。注意:BIT1 语义下每 32 个专家打包为 1 个 u32(测试 `tests/kernels/softmax_topk.py:37-38` 中 `idx = elem / 64`、`1 << (elem % 64)` 是因为 BIT1 张量在 torch 侧被视作 int64 承载) |
...
dtype 分派见 `csrc/op.cpp:1281-1291`:要求 scores 与 weightsMap 同 dtype、indices 为 INT32、routingMap 为 BIT1。纯向量核。
