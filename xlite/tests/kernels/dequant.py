...
for m, n in cases:
    for has_scale in [False, True]:
        ...
        if has_scale:
            standard = (inout.to(torch.float) * scales.unsqueeze(-1)).to(torch.bfloat16)
        else:
            standard = inout.to(torch.float).to(torch.bfloat16)
        ...
        try:
            torch.testing.assert_close(standard, inout, atol=1e-5, rtol=1e-3)
