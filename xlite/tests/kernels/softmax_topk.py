...
scores = torch.randn(n_tokens, n_routed_experts, dtype=torch.float32, device="npu:0")
...
xlite_weights_out = torch.empty(n_tokens, n_routed_experts, dtype=torch.float32, device="npu:0")

standard_indices_out = torch.zeros(n_tokens, n_indices_map, dtype=torch.int64, device="npu:0")
standard_weights_out = torch.zeros(n_tokens, n_routed_experts, dtype=torch.float32, device="npu:0")
standard_softmax = scores.softmax(dim=-1, dtype=torch.float32)
