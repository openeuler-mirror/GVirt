...
for test_dtype in dtype_list:
    for has_bias in [True, False]:
        with torch.device("npu"):
            ...
            bias = torch.randn(DIM, dtype=test_dtype).to(torch.float)
        ...
        if has_bias:
            rmsnorm(rt, x, weight, bias, y, NORMEPS)
        else:
...
for test_dtype in dtype_list:
    for has_bias in [True, False]:
        with torch.device("npu"):
            ...
            x_standard = x.clone()
            weight = torch.randn(DIM, dtype=test_dtype)
            bias1 = torch.randn(DIM, dtype=test_dtype).to(torch.float)
            bias2 = torch.randn(DIM, dtype=test_dtype).to(torch.float)
            y = x.clone()
        ...
        torch.npu.synchronize()
        if has_bias:
            rmsnorm(rt, x, weight, bias1, y, NORMEPS, DIM, CNT)
            rmsnorm(rt, x, weight, bias2, y, NORMEPS, DIM, 1, DIM * CNT)
        else:
