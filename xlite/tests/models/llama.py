...
class Llama(nn.Module):
    ...
    def prepare_xlite_attnmeta_v2(self, tokens: torch.Tensor, start_pos: int):
        ...
        # slot_mapping: [batchedTokens] int32
        # slot[i][j] = block_tables[i][pos[j]//bs] * bs + pos[j]%bs, pos[j]=cached_lens[i]+j
        positions_per_sample = np.arange(start_pos, start_pos + seqlen, dtype=np.uint32)
        block_idx = positions_per_sample // block_size  # [seqlen]
        block_id_in_table = bt_padded[np.arange(batch)[:, None], block_idx]  # [batch, seqlen]
        offset_in_block = positions_per_sample % block_size  # [seqlen]
