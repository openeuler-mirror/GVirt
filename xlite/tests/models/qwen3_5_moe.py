...
if forward_backend == "xlite":
    block_size = 128
    from xlite._C import Runtime, ModelConfig, AttnMeta, AttnMetaV2, AttnHybrid, Model, ScoringFuncSoftmax
    import numpy as np
    from tests.models.xlite_utils import prepare_xlite_attnmeta_v2


def _mha_qkv_weight(attn, head_dim: int, n_heads: int):
    """Pack fused MHA weights as [Q | K | V | Gate] for xlite ForwardAttnMHA."""
...
class ModelArgs:
    ...
    shared_expert_inter_dim: int = 512
...
class RMSNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return ((1 + self.weight.float()) * x).to(input_dtype)
...
def precompute_freqs_cis(dim: int, end: int, theta: float = 10000.0):
    ...
    cos_cache = emb.cos()  # fp32: rope_and_cache cos/sin contract
    sin_cache = emb.sin()
...
def apply_rotary_emb(x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, rotary_dim: int) -> torch.Tensor:
    ...
    x_embed = ((x_rot * cos) + (rotate_half(x_rot) * sin)).to(x.dtype)
...
class LinearAttn(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.norm = RMSNormGated(self.head_v_dim, args.norm_eps)
    ...
    def _torch_recurrent_gated_delta_rule(self, query, key, value, g, beta, initial_state=None, output_final_state=False, use_qk_l2norm=True):
        """Recurrent gated delta rule for decode mode."""
        initial_dtype = query.dtype

        # L2 normalize q and k (CRITICAL for correct behavior)
        if use_qk_l2norm:
            query = self._l2norm(query.float(), dim=-1)
            key = self._l2norm(key.float(), dim=-1)
...
class Gate(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        self.weight = nn.Parameter(torch.zeros(args.n_routed_experts, args.dim, dtype=dtype))
        self.norm_topk_prob = args.norm_topk_prob

    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        scores = linear(x, self.weight)
        scores_float = scores.softmax(dim=-1, dtype=torch.float)

        weights, indices = torch.topk(scores_float, self.topk, dim=-1)
...
class MoE(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        if args.n_shared_experts > 0:
            self.shared_expert = MLP(args.dim, args.shared_expert_inter_dim)
            self.shared_expert_gate = Linear(args.dim, 1, bias=False)
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        if self.is_full_attention:
            attn_out = self.self_attn(self.input_layernorm(x), start_pos, freqs_cis, mask)
        else:
            attn_out = self.linear_attn(self.input_layernorm(x), start_pos)

        x = x + attn_out
        post_ln = self.post_attention_layernorm(x)
...
class Qwen3_5MoE(nn.Module):
    ...
    def load_weights(
        ...
        for name, loaded_weight in hf_model_weights_iterator(model_path):
            ...
            if "experts.gate_up_proj" in name:
                ...
                experts_start = moe_ep_id * n_local_experts
                experts_end = (moe_ep_id + 1) * n_local_experts
                loaded_weight = loaded_weight[experts_start:experts_end]
                layer = self.layers[layer_id]
                if is_layer_moe(self.args, layer_id):
                    for i in range(layer.mlp.experts_start_idx, layer.mlp.experts_end_idx):
                        expert_idx = i - layer.mlp.experts_start_idx
                        if expert_idx < loaded_weight.shape[0]:
                            param = layer.mlp.experts[i].gate_up_proj.weight
                            param.data.copy_(loaded_weight[expert_idx])
            ...
            if "experts.down_proj" in name:
                ...
                if is_layer_moe(self.args, layer_id):
                    for i in range(layer.mlp.experts_start_idx, layer.mlp.experts_end_idx):
                        expert_idx = i - layer.mlp.experts_start_idx
                        if expert_idx < loaded_weight.shape[0]:
                            param = layer.mlp.experts[i].down_proj.weight
                            param.data.copy_(loaded_weight[expert_idx])
                torch.npu.empty_cache()
                continue
            ...
            # Handle shared_expert down_proj - RowParallelLinear with special sharding
            if "shared_expert.down_proj" in name:
                # For TP: checkpoint: [dim, shared_inter_dim], model: [dim, shared_inter_dim//world_size]
                # For EP: shared_expert is replicated across EP ranks, only TP sharding applies
                shard_size = self.args.shared_expert_inter_dim // world_size
                loaded_weight = convert_pyslice_to_tensor(loaded_weight[:, shard_size * rank:shard_size * (rank + 1)])
                param.data.copy_(loaded_weight)
                continue

            # Handle shared_expert_gate - Linear (no sharding needed)
            if "shared_expert_gate" in name:
                loaded_weight = convert_pyslice_to_tensor(loaded_weight)
                param.data.copy_(loaded_weight)
                continue

            # Handle MoE expert down_proj
            if "down_proj" in name and "experts" in name and "shared_expert" not in name:
            ...
            # Handle non-MoE MLP down_proj
            if "down_proj" in name and not is_layer_moe(self.args, int(name.split(".")[2]) if "layers" in name else -1):
                load_tensor_parallel_weights(param, loaded_weight,
        ...
        # transpose experts weights for xlite
        if forward_backend == "xlite":
            ...
            for layer_id, layer in enumerate(self.layers):
                ...
                if not is_layer_moe(self.args, layer_id):
                    ...
                    if hasattr(layer.mlp, 'shared_expert'):
                        layer.mlp.shared_expert.gate_up_proj.weight.data = matrix_nd2nz(layer.mlp.shared_expert.gate_up_proj.weight)
                        layer.mlp.shared_expert.down_proj.weight.data = matrix_nd2nz(layer.mlp.shared_expert.down_proj.weight)
                        layer.mlp.shared_expert_gate.weight.data = matrix_nd2nz(layer.mlp.shared_expert_gate.weight)
            torch.npu.empty_cache()
        ...
        if forward_backend == "xlite":
            ...
            if self.multi_task_parallel:
                self.xlite_rt_list = [self.xlite_rt, Runtime(local_rank, 0, self.global_rank,
                                                             self.tp_size, self.dp_size, 1, self.args.moe_ep_size)]
    ...
    def init_xlite_model(self, args: ModelArgs):
        ...
        self.xlite_model.se_up_gate = [
            self.layers[i].mlp.shared_expert.gate_up_proj.weight
            for i in range(args.n_layers)
            if is_layer_moe(self.args, i) and hasattr(self.layers[i].mlp, 'shared_expert')
        ]
        self.xlite_model.se_down = [
            self.layers[i].mlp.shared_expert.down_proj.weight
            for i in range(args.n_layers)
            if is_layer_moe(self.args, i) and hasattr(self.layers[i].mlp, 'shared_expert')
        ]
        self.xlite_model.se_gate = [
            self.layers[i].mlp.shared_expert_gate.weight
            for i in range(args.n_layers)
            if is_layer_moe(self.args, i) and hasattr(self.layers[i].mlp, 'shared_expert')
        ]
