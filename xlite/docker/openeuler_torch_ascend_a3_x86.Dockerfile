...
RUN curl -s -k "https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%208.3.RC2/Ascend-cann-toolkit_8.3.RC2_linux-x86_64.run" -o Ascend-cann-toolkit.run && \
    chmod a+x *.run && \
    bash /root/Ascend-cann-toolkit.run --install -q && \
    rm /root/*.run
RUN curl -s -k "https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%208.3.RC2/Atlas-A3-cann-kernels_8.3.RC2_linux-x86_64.run" -o Atlas-A3-cann-kernels.run && \
    chmod a+x *.run && \
    bash /root/Atlas-A3-cann-kernels.run --install -q && \
    rm /root/*.run
