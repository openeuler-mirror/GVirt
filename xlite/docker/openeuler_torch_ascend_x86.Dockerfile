...
RUN curl -s -k "https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%208.3.RC2/Ascend-cann-toolkit_8.3.RC2_linux-x86_64.run" -o Ascend-cann-toolkit.run && \
    ...
    rm /root/*.run
RUN curl -s -k "https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%208.3.RC2/Ascend-cann-kernels-910b_8.3.RC2_linux-x86_64.run" -o Ascend-cann-kernels-910b.run && \
    chmod a+x *.run && \
